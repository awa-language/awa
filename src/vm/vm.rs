    ...
    pub fn run(&mut self) {
        ...
        match instr {
            ...
            Instruction::Equal => {
                ...
                match (a, b) {
                    (Value::Int(x), Value::Int(y)) => self.stack.push(Value::Int((x == y) as i64)),
                    (Value::Float(x), Value::Float(y)) => {
                        self.stack.push(Value::Int((x == y) as i64))
                    }
                    (Value::Str(x), Value::Str(y)) => self.stack.push(Value::Int((x == y) as i64)),
            ...
            Instruction::Println => {
                let val = self.stack.pop().expect("Stack underflow on Println");
                println!("{:?}", val);
            }
